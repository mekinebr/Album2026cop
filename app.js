...
function matchesText(x,q){
  ...
  return norm(x.code).includes(q)||norm(x.id).includes(q)||norm(x.team).includes(q)||norm(x.group).includes(q)||String(x.page).includes(q)||norm(x.code.replace('-','')).includes(raw)||norm(x.id.replace('-','')).includes(raw)||norm('pagina '+x.page).includes(q);
}
...
function findSticker(q){q=String(q||'').trim(); if(!q) return null; const qn=norm(q).replace(/\s/g,''); return stickers.find(x=>norm(x.id)===qn||norm(x.code)===norm(q)||norm(x.code.replace('-',''))===qn||norm(x.id.replace('-',''))===qn)||stickers.find(x=>matchesText(x,q))}
...
function renderDashboard(){const c=counts(); const p=pct(c.have,c.total); $('#haveCount').textContent=c.have; $('#repeatCount').textContent=c.repeat; $('#missingCount').textContent=c.missing; $('#totalCount').textContent=c.total; $('#homeRepeatCount').textContent=c.repeat; $('#homeMissingCount').textContent=c.missing; $('#percent').textContent=p+'%'; $('#barFill').style.width=p+'%'; $('#progressText').textContent=`${c.have} de ${c.total} figurinhas marcadas como tenho`}
function openView(name){$$('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===name)); $$('.view').forEach(v=>v.classList.remove('active')); $('#'+name).classList.add('active'); if(name==='sheets') renderSheets(); if(name==='trades') renderTrades(); if(name==='album') renderCards(); window.scrollTo({top:0,behavior:'smooth'})}
...
function filterTo(q,status='all'){activeGroup=''; activeTeam=''; openView('album'); $('#search').value=q; $('#statusFilter').value=status; renderCards()}
function renderGroupSelector(){
  $('#groupSelector').innerHTML=GROUPS.map(g=>{const items=stickers.filter(x=>x.group===g.name); const c=counts(items); return `<button class="group-btn ${activeGroup===g.name?'active':''}" onclick="selectGroup('${g.name}')"><b>${g.name}</b><br><small>✅ ${c.have} / ${c.total}</small></button>`}).join('');
}
function renderTeamSelector(){
  const groupsToShow = activeGroup ? GROUPS.filter(g=>g.name===activeGroup) : GROUPS.slice(0,3);
  $('#teamSelector').innerHTML=groupsToShow.flatMap(g=>g.teams).map(t=>{
    const team=t[0]; const items=stickers.filter(x=>x.team===team); const c=counts(items); const p=pct(c.have,c.total);
    return `<button class="team-card ${activeTeam===team?'active':''}" onclick="selectTeam('${team}')"><header><b>${team}</b><span>${p}%</span></header><div class="smallbar"><i style="width:${p}%"></i></div><div class="mini-counts">✅ ${c.have} · ❌ ${c.missing} · 🔁 ${c.repeat} · ⭕ ${c.none}</div></button>`;
  }).join('');
}
function renderTeamProgress(){
  $('#teamProgress').innerHTML=TEAM_PAGES.slice(0,48).map(t=>{const team=t[0]; const items=stickers.filter(x=>x.team===team); const c=counts(items); const p=pct(c.have,c.total); return `<div class="team-row" onclick="selectTeam('${team}')"><header><span>${team}</span><span>${p}%</span></header><div class="smallbar"><i style="width:${p}%"></i></div><small class="muted">✅ ${c.have} · ❌ ${c.missing} · 🔁 ${c.repeat} · Total ${c.total}</small></div>`}).join('')
}
function renderActiveFilters(){
  const chips=[]; if(activeGroup) chips.push(`📌 ${activeGroup}`); if(activeTeam) chips.push(`⚽ ${activeTeam}`); const st=$('#statusFilter')?.value; if(st&&st!=='all') chips.push(`${statusEmoji(st)} ${statusLabel(st)}`); const q=$('#search')?.value; if(q) chips.push(`🔎 ${q}`);
...
function renderCards(){
  ...
  let title='Figurinhas'; if(activeTeam) title=activeTeam; else if(activeGroup) title=activeGroup;
  $('#albumTitle').textContent=title;
  $('#albumSubtitle').textContent=`${filtered.length} figurinhas encontradas · ✅ ${c.have} · ❌ ${c.missing} · 🔁 ${c.repeat} · ⭕ ${c.none}`;
  renderActiveFilters();
...
function renderSheets(){ $('#sheetGrid').innerHTML=Array.from({length:TOTAL_PAGES},(_,i)=>i+1).map(p=>`<div class="sheet" onclick="openSheet(${p})"><img src="${pageImage(p)}" loading="lazy" alt="Página ${p}"><div>Página ${String(p).padStart(3,'0')} · ${pageTeam(p)[0]}</div></div>`).join('') }
...
function renderQuick(x){ if(!x){$('#quickResult').innerHTML='<div class="notice">Figurinha não encontrada. Use exemplo: ARG-01, BRA-12, Brasil ou Grupo A.</div>';return} $('#quickResult').innerHTML=cardHTML(x)}
...
$('#quickCode').addEventListener('input',e=>{const q=e.target.value.trim(); if(q.length>=2){ const group=GROUPS.find(g=>norm(g.name).includes(norm(q))); if(group){selectGroup(group.name); return;} renderQuick(findSticker(q)); }});
